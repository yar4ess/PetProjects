...
public class Group_by extends Command {
    ...
    public Group_by(Console console, CollectionManager collectionManager, CommandManager commandManager) {
        super("group_by field", "группировка объектов по переданному полю и вывод топ-3 объектов по этому полю");
    ...
    public CommandMessage execution(String args, Object obj) {
        ...
        String field = args.split(" ")[0];
        if (!field.isEmpty()){
            if (groups.containsKey(args)){
                return new CommandMessage("Количество групп и их топ успешно выведены!" + "\n" + groups.get(field));
            } else {
                return new CommandMessage(400, "Такого поля не существует");
            }
        } else {
            return new CommandMessage(400, "Неверное количество аргументов");
        }
    }
}